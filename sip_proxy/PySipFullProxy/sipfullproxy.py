...
class UDPHandler(socketserver.BaseRequestHandler):
    ...
    def getOrigin(self):
        ...
        for line in self.data:
            if type(line) == bytes:
                line = line.decode("utf-8")
            print(f"line: {line}\nfrom: {rx_from.search(line)}\ncfrom: {rx_cfrom.search(line)}")
            if rx_from.search(line) or rx_cfrom.search(line):
                print(f"rx_uri: {rx_uri}")
                md = rx_uri.search(line)
                if md:
    ...
    def processRequest(self):
        ...
        if len(self.data) > 0:
            request_uri = self.data[0].decode()
    ...
    def handle(self):
        ...
        if rx_request_uri.search(request_uri.decode()) or rx_code.search(request_uri.decode()):
            showtime()
