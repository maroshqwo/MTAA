...
class TCPHandler(socketserver.BaseRequestHandler):
    ...
    def processInvite(self):
        ...
        if len(origin) == 0 or not origin in registrar:
             self.sendResponse("400 Chyba požiadavky")
             return
        ...
        if len(destination) > 0:
            ...
            if destination in registrar and self.checkValidity(destination):
                ...
                try:
                    ...
                    self.sendResponse("480 Nedostupné")
            else:
                self.sendResponse("480 Nedostupné")
        else:
...
def writeLog(data):
    ...
    with open("log.txt", "a") as f:
        if method == "INVITE" or method == "ACK" or method == "BYE":
            name_from = data[2].split(" ")[1].split(";")[0]
            name_to = data[3].split(" ")[1].split(";")[0]
            call_id = data[5].split(" ")[1]
            if len(call_id) > 15:
                return
            if method == "BYE":
                f.write(f"{time} - [CALL-ID: {call_id}] Call ended [{method}] FROM: {name_from}    TO: {name_to}\n")
            else:
                f.write(f"{time} - [CALL-ID: {call_id}] Call incoming [{method}] FROM: {name_from}    TO: {name_to}\n")
